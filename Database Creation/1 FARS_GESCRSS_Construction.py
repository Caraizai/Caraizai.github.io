...
#---------- Code used to download, extract and merge files needed for FARS, GES and CRSS ---------------------------####
...
person = pd.read_csv(path_fars+'FARS'+str(endyear)+'NationalCSV/Person.CSV', encoding='latin-1', low_memory=False)
# Variables desired for analysis from the person files
list_per = ['ST_CASE','VEH_NO','PER_NO','AGE','SEX','YEAR','PER_TYP','INJ_SEV','DRINKING','DRUGS']
...
for year in range(endyear-1,iniyear-1,-1):
    person = pd.read_csv(path_fars+'FARS'+str(year)+'NationalCSV/Person.CSV', encoding='latin-1', low_memory=False)
    person['YEAR'] = year
...
# Start the merge process using the last year of data in period of study
vehicle = pd.read_csv(path_fars+'FARS'+str(endyear)+'NationalCSV/Vehicle.CSV', encoding='latin-1', low_memory=False)
...
list_vec = ['ST_CASE','VEH_NO','YEAR','NUMOCCS','MAKE', 'MODEL', 'MOD_YEAR', 'HIT_RUN', 'BODY_TYP',
            'DEFORMED','SPEC_USE', 'TRAV_SP', 'DR_DRINK', 'DR_ZIP', 'CDL_STAT',
            'PREV_ACC', 'PREV_SUS', 'PREV_DWI', 'PREV_SPD','SPEEDREL', 'DR_SF1']
df_vec = vehicle[[*list_vec]]
for year in range(endyear-1,iniyear-1,-1):
    vehicle = pd.read_csv(path_fars+'FARS'+str(year)+'NationalCSV/Vehicle.CSV', encoding='latin-1', low_memory=False)
    vehicle = vehicle.rename(columns={'PREV_SUS1': 'PREV_SUS'})
...
# Start the merge process using the last year of data in period of study
accident = pd.read_csv(path_fars+'FARS'+str(endyear)+'NationalCSV/Accident.csv', encoding='latin-1', low_memory=False)
list_acc = ['ST_CASE', 'STATENAME', 'YEAR', 'COUNTY', 'HARM_EV', 'HOUR', 'WEATHER']
df_acc = accident[[*list_acc]]
# Before 2016 the state number was used instead of STATENAME. We use this list to replace the numerical value,
# with the state name
state_name = pd.read_csv(path_aux + 'Statenumbers.CSV')
state_name['STATE'] = state_name.STATE.astype(str)
for year in range(endyear-1,iniyear-1,-1):
    accident = pd.read_csv(path_fars+'FARS'+str(year)+'NationalCSV/Accident.CSV', encoding='latin-1', low_memory=False)
    if year < 2016:
        accident['STATE'] = accident.STATE.astype(str)
...
df_per['KEY'] = df_per.YEAR.astype(str) + df_per.ST_CASE.astype(str) + df_per.VEH_NO.astype(str)
df_vec['KEY'] = df_vec.YEAR.astype(str) + df_vec.ST_CASE.astype(str) + df_vec.VEH_NO.astype(str)
...
df_acc['KEY'] = df_acc.YEAR.astype(str) + df_acc.ST_CASE.astype(str)
...
# Merge the Person files
# Start the merge process using the last year of data in period of study
person = pd.read_csv(path_gescrss+'CRSS'+str(endyear)+'CSV/person.csv', encoding='latin-1', low_memory=False)
# Variables desired for analysis from the person files
list_per = ['CASENUM','VEH_NO','PER_NO','AGE','SEX','YEAR','SOURCE','PER_TYP','INJ_SEV','DRINKING','DRUGS']
# Add a new variable with the corresponding YEAR in order to make annual distinctions
person['YEAR'] = endyear
...
for year in range(endyear-1,iniyear-1,-1):
    if year>=2016:
        person = pd.read_csv(path_gescrss + 'CRSS' + str(year) + 'CSV/Person.CSV', encoding='latin-1', low_memory=False)
    elif year==2015:
        person = pd.read_csv(path_gescrss + 'GES' + str(2015) + 'CSV/Person.CSV', encoding='latin-1', low_memory=False)
    elif year == 2010:
        person = pd.read_csv(path_gescrss + 'GES' + str(year) + 'TXT/TXT/Person.txt', delimiter="\t", low_memory=False)
    elif (year>=2009)&(year<=2014):
        person = pd.read_csv(path_gescrss + 'GES' + str(year) + 'TXT/Person.txt', delimiter="\t", low_memory=False)
        ...
        person = pd.read_csv(path_aux + 'GES2001CSV/Person.CSV', encoding='latin-1', low_memory=False)
    if year>=2016:
        person['SOURCE'] = 'CRSS'
    else:
        person['SOURCE'] = 'GES'
...
# Merge the Vehicle files
# Start the merge process using the last year of data in period of study
vehicle = pd.read_csv(path_gescrss+'CRSS'+str(endyear)+'CSV/vehicle.csv', encoding='latin-1', low_memory=False)
vehicle['YEAR'] = endyear
list_vec = ['CASENUM','VEH_NO','YEAR','NUMOCCS','MAKE', 'MODEL', 'MOD_YEAR', 'HIT_RUN', 'BODY_TYP',
            'DEFORMED','SPEC_USE', 'TRAV_SP', 'DR_ZIP','SPEEDREL', 'DR_SF1']
df_vec = vehicle[[*list_vec]]
for year in range(endyear-1,iniyear-1,-1):
    if year>=2016:
        vehicle = pd.read_csv(path_gescrss + 'CRSS'+str(year)+'CSV/vehicle.CSV', encoding='latin-1', low_memory=False)
    elif year==2015:
        vehicle = pd.read_csv(path_gescrss + 'GES' + str(2015) + 'CSV/vehicle.CSV',encoding='latin-1', low_memory=False)
    elif year == 2010:
        vehicle = pd.read_csv(path_gescrss + 'GES' + str(year) + 'TXT/TXT/vehicle.txt', delimiter="\t", low_memory=False)
    elif (year>=2009)&(year<=2014):
        vehicle = pd.read_csv(path_gescrss + 'GES'+ str(year)+'TXT/vehicle.txt', delimiter = "\t", low_memory=False)
    elif (year>=2002)&(year<=2008):
        vehicle = pyreadstat.read_sas7bdat(path_gescrss + 'GES'+ str(year)+'SAS/vehicle.sas7bdat')[0]
    elif year==2001:
        # WARNING: For Year==2001, the datafile .sd2 was converted to .csv with SPSS
        vehicle = pd.read_csv(path_aux + 'GES2001CSV/vehicle.CSV',encoding='latin-1', low_memory=False)
...
# Merge the Accident files
# Start the merge process using the last year of data in period of study
accident = pd.read_csv(path_gescrss+'CRSS'+str(endyear)+'CSV/accident.csv', encoding='latin-1', low_memory=False)
accident['YEAR'] = endyear
list_acc = ['CASENUM','YEAR','HARM_EV','HOUR','WEATHER','STRATUM', 'REGION', 'PSU','PJ','WEIGHT']
df_acc = accident[[*list_acc]]
for year in range(endyear-1,iniyear-1,-1):
    if year>=2016:
        accident = pd.read_csv(path_gescrss + 'CRSS'+str(year)+'CSV/accident.CSV', encoding='latin-1', low_memory=False)
    elif year==2015:
        ...
        accident = pd.read_csv(path_gescrss + 'GES'+ str(year)+'TXT/accident.txt', delimiter = "\t", low_memory=False)
    elif (year>=2002)&(year<=2008):
        accident = pyreadstat.read_sas7bdat(path_gescrss + 'GES'+ str(year)+'SAS/accident.sas7bdat')[0]
    elif year == 2001:
        # WARNING: For Year==2001, the datafile .sd2 was converted to .csv with SPSS
        accident = pd.read_csv(path_aux + 'GES2001CSV/accident.CSV',encoding='latin-1')
...
df_per['KEY'] = df_per.YEAR.astype(str) + df_per.CASENUM.astype(str) + df_per.VEH_NO.astype(str)
df_vec['KEY'] = df_vec.YEAR.astype(str) + df_vec.CASENUM.astype(str) + df_vec.VEH_NO.astype(str)
df = mergeLeftInOrder(x=df_per,y=df_vec, on="KEY")
...
df_acc['KEY'] = df_acc.YEAR.astype(str) + df_acc.CASENUM.astype(str)
df = mergeLeftInOrder(x=df,y=df_acc, on="KEY")
